 ...
 *
 * src : pointer to input TBmpFrame (const, non-NULL).
 * dst : pointer to output TBmpFrame (non-NULL, must have valid pixel buffer).
 *
 ...
 *
 * Thread safety: These functions are thread-safe as long as each thread uses separate src and dst instances.
 ...
 * out_w, out_h : receives the output dimensions (non-NULL).
 *
 ...
 *
 * src       : pointer to input TBmpFrame (const, non-NULL, must be fully decoded).
 * dst       : pointer to output TBmpFrame (non-NULL, must have pixel buffer of at least out_w * out_h TBmpRGBA elements).
 *             Use tbmp_rotate_output_dims to determine the required size.
 ...
 * Not available when TBMP_NO_FLOAT is defined.
 *
 * Thread safety: This function is thread-safe as long as each thread uses separate src and dst instances.
 * Ownership: The caller retains ownership of all buffers and structs.
 */
